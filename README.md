...
[Demo](https://nav.dogxi.me) · [快速开始](#快速开始) · [功能特性](#功能特性) · [技术亮点](#技术亮点)
...
<!-- 截图说明（添加后删除此注释）：
  1. 首页全貌（亮色）  — 浏览器窗口最大化，展示卡片网格 + Header + 分类筛选条
  2. 首页全貌（暗色）  — 切换暗色后同角度截图，与亮色并排放
  3. 搜索状态         — 输入关键词，展示卡片高亮 + 搜索引擎卡片
  4. 命令面板         — ⌘K 打开状态，输入关键词，展示搜索结果列表
  5. 右键菜单         — 鼠标悬停在卡片上触发右键菜单
  建议使用 1280×800 窗口截图，保存为 docs/screenshots/ 目录下的 PNG 文件
-->

| 亮色                                         | 暗色                                        |
| -------------------------------------------- | ------------------------------------------- |
| ![亮色主页](docs/screenshots/home-light.png) | ![暗色主页](docs/screenshots/home-dark.png) |
...
| **Ctrl+1~9 快捷打开** | 搜索状态下，前 9 个结果可用数字键直接打开                     |
...
| `⌘K` / `Ctrl+K`     | 打开命令面板                     |
| 任意可打印字符      | 聚焦搜索框（非输入状态）         |
| `Ctrl+1` ~ `Ctrl+9` | 搜索状态下直接打开第 N 个结果    |
| `↑` `↓`             | 命令面板导航                     |
...
| 指标              | 数值   | 说明                                    |
| ----------------- | ------ | --------------------------------------- |
| Performance       | 90+    | 生产构建 preview 模式                   |
...
```bash
bun run release          # patch: 0.8.0 -> 0.8.1
bun run release:minor    # minor: 0.8.0 -> 0.9.0
bun run release:major    # major: 0.8.0 -> 1.0.0
```

脚本会自动完成：构建检查 → 更新 `package.json` 版本号 → `git commit` + `git tag` → `git push` → 打开 GitHub Release 草稿页（填写内容后手动发布）。
